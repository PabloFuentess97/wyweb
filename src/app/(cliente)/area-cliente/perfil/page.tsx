...
export default async function PerfilPage() {
  ...
  // Cargamos el user real de DB (la sesión podría tener datos cacheados del JWT)
  const [user] = await db
    .select({
      id: users.id,
      name: users.name,
    ...
    <div className="px-4 md:px-6 lg:px-8 py-8 max-w-4xl mx-auto">
      <header className="flex flex-col gap-2 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-1">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.08em] text-[var(--color-fg-muted)]">
            CLIENTE DESDE · {memberSince.toUpperCase()}
          </span>
        </div>
      </header>
      ...
      <div className="flex flex-col gap-8">
        ...
        <Section
          ...
          description="Tu nombre, idioma y aspecto del producto. Los cambios se guardan tras pulsar 'Guardar'."
        >
          <ProfileForm
            defaults={{
              name: user.name,
        ...
        <Section
          ...
          title="Contraseña"
          description="Mínimo 12 caracteres y fuerza buena o superior. Usa un gestor de contraseñas si puedes."
        >
          {hasPassword ? (
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-[var(--radius-3)] border border-dashed border-[var(--color-border-strong)] bg-[var(--color-bg-subtle)] p-5">
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-[var(--color-fg)] leading-relaxed">
                  La activación de 2FA con TOTP estará disponible en una próxima fase del
                  producto. Mientras tanto, mantén una contraseña fuerte y única.
                </p>
        ...
        >
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
            ...
            <DataRow label="Customer(s) asociado(s)">
              <span className="font-mono text-xs tnum text-[var(--color-fg-muted)]">
                {session.user.customerIds.length === 0
                  ? 'NINGUNO'
                  : session.user.customerIds.length === 1
                    ? '1 CUSTOMER'
                    : `${session.user.customerIds.length} CUSTOMERS`}
              </span>
            </DataRow>
          </dl>
        </Section>
